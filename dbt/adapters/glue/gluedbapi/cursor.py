...
class GlueCursor:
    ...
    def execute(self, sql, bindings=None):
        ...
        self.statement = GlueStatement(
            client=self.connection.client, session_id=self.connection.session_id, code=self.code
        )
        ...
        try:
            ...
            logger.exception(
                f"Error in GlueCursor (session_id={self.connection.session_id}) execute: {e}"
            )
            ...
            if status == "ok":
                try:
                    self.response = json.loads(
                        output.get("Data", {}).get("TextPlain", None).strip()
                    )
    ...
    def fetchall(self):
        ...
        if self.response:
            records = []
            logger.debug(
                f"GlueCursor fetchall results={self.columns}, use_arrow={self.connection.use_arrow}"
            )
    ...
    def fetchmany(self, limit: Optional[int]):
        ...
        if self.response:
            ...
            i = 0
            logger.debug(
                f"GlueCursor fetchmany results={self.columns}, use_arrow={self.connection.use_arrow}"
            )
            for item in self.result.get("results", []):
                record = []
                for column in self.columns:
    ...
    def fetchone(self):
        ...
        if self.response:
            ...
            try:
                record = []
                logger.debug(
                    f"GlueCursor fetchone results={self.columns}, use_arrow={self.connection.use_arrow}"
                )
    ...
    def get_response(self) -> AdapterResponse:
        ...
        if self.statement:
            ...
            return AdapterResponse(
                _message=r.get("Statement", {}).get("State", ""), code=self.sql, **r
            )
